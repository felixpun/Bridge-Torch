...
def uniform(traj, *, reached_proportion):
    ...
    traj_len = traj["terminals"].shape[0]
    ...
    low = np.arange(traj_len, dtype=np.float32) + 1
    goal_idxs = (rand * (traj_len - low) + low).astype(np.int64)
    ...
    goal_reached_mask = goal_reached_mask | (np.arange(traj_len) == traj_len - 1)
    ...
    # select goals
    traj["goals"] = dict(
        (key, value[goal_idxs, ...]) for key, value in traj["next_observations"].items()
    )

    # reward is 0 for goal-reaching transitions, -1 otherwise
    traj["rewards"] = np.where(goal_reached_mask, 0, -1)

    # add masks
    traj["masks"] = ~(traj["terminals"])
